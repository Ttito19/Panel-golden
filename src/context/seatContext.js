...
const SeatProvider = (props) => {
  ...
  //Context
  const { dataFromDocument, searchSeatDesignFromId } = useContext(
    SeatDesignContext
  );
        ...
        for (let i = 0; i < ts.length; i++) {
          if (ts[i].name) {
            question = window.confirm(
              "Al actualizar la estructura todos los nombres se resetearan tambien. ¿Esta Seguro de esto?"
            );
            break;
          }
        }
...
export { SeatProvider, SeatContext };
